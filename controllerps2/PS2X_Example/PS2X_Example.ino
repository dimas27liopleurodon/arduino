...
void setup(){
 ...
 error = ps2x.config_gamepad(5,4,3,2,false,false);   //setup  (clock, c ommand, attention, data, Pressures?, Rumble?) check for error
...
void loop(){
 ...
 if(type == 2){ //Guitar Hero Controller
   ...
   if(ps2x.ButtonPressed(ORANGE_FRET))
    ...
    if(ps2x.Analog(PSS_LX) != 128) //if right analog stick is moved on X axis right-left
    {
      Serial.println("Right Stick Moves!");
      if(ps2x.Analog(PSS_LX) > 128)
      {
        myservo.write( map(ps2x.Analog(PSS_LX), 0,127,0,89));
        delay(15);
      }
      if(ps2x.Analog(PSS_LX) < 128)
      {
        myservo.write( map(ps2x.Analog(PSS_LX), 129,255,91,180));
        delay(15);
      }
    }
    if(ps2x.Analog(PSS_LX) == 128) //if right analog stick is center
    {
    ...
    if(ps2x.Analog(PSS_RX) != 128) //if right analog stick is moved on X axis right-left
    {
      ...
      {
        myservo1.write( map(ps2x.Analog(PSS_RX), 129,255,91,180));
        delay(15);
      }
      if(ps2x.Analog(PSS_RX) < 128)
      {
        myservo1.write( map(ps2x.Analog(PSS_RX), 0,127,0,89));
        delay(15);
      }
    }
