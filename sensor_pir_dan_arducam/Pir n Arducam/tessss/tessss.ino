...
static IPAddress ip(192, 168, 4, 1); // static IP used for browser access: http://IPaddress
static IPAddress gateway(192, 168, 4, 15);
static IPAddress subnet(25 5,255, 255, 0);
int pirPin = D3; // nama alias pirpin pada pin 1
String deteksi;
...
void pir() {
  ...
  delay(50);
    ...
    String pir = "<!DOCTYPE html><html><head><title>" + title + "</title><meta charset=\"utf-8\" /><meta name=\"viewport\" content=\"width=device-width\" /><link href='https://fonts.googleapis.com/css?family=Advent+Pro' rel=\"stylesheet\" type=\"text/css\"><style>\n";
    pir += "html {height: 100%;}";
    pir += "div {color: #fff;font-family: 'Advent Pro';font-weight: 400;left: 50%;position: absolute;text-align: center;top: 50%;transform: translateX(-50%) translateY(-50%);}";
    pir += "h2 {font-size: 90px;font-weight: 400; margin: 0}";
    pir += "body {height: 100%;}";
    pir += ".cold {background: linear-gradient(to bottom, #7abcff, #0665e0 );}";
    pir += ".mediumhot {background: linear-gradient(to bottom, #81ef85,#057003);}";
    pir += ".hot {background: linear-gradient(to bottom, #fcdb88,#d32106);}";
    pir += "</style></head><body class=\"" + cssClass + "\"><div><h1>" + title +  "</h1><h2>" + deteksi + "</div></body></html>";
    pir += "<meta http-equiv=refresh content=7;>";
    server.send(200, "text/html", pir);
}
...
void handleNotFound() {
  ...
  else
  {
    ...
    message += " &nbsp;  <a style=\"font-size:12px; font-weight:bold;\" href=\"http://" + ipStr +"/PIR\">PIR</a>";
